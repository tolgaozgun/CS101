...
public class TestLibraryBook {
	...
	public static void main(String[] args) throws ParseException {
		...
		bookThree = new LibraryBook("One Hundred Years of Solitude", "Gabriel García Márquez");
		
		// Step 2: Continue the loop until the user wants to stop.
		while ( firstRun || (scan.hasNext() && scan.nextLine().toLowerCase().contains("y")) ) {
			firstRun = false;
			
			// Step 3: Ask the user for their commands. If the book is not valid
			// loop this sequence.
			while ( userBook == null ) {
				// Step 3.1: Print the instructions to the user.
				printHelp();
				
				// Step 3.2: Get the instructions from the user.
				userInput = scan.nextLine();
				
				// User 3.3: Get the command and the book.
			...
			// Step 4: Check if command is valid. If the command is one of
			// 'borrow, return, status' use the valid functions.
			if ( userCommand.equalsIgnoreCase("borrow") ) {
				// Step 4.1.1: If the book is loaned, tell user it is currently loaned
				if ( userBook.onLoan() ) {
					System.out.println(userBook.getTitle() + " is already borrowed! Come back later.");
				} else {
					// Step 4.1.2: If the book is not loaned, print the instructions
					// for getting a date.
					enterDate();
					
					// Step 4.1.3: From the user, get the date  
				...
				// Step 4.2.1: If the book is not loaned, tell user it is not loaned.
				if( !userBook.onLoan() ) {
					System.out.println(userBook.getTitle() + " is not borrowed!");
				} else {
					// Step 4.2.2: If it is not loaned, return the book and print
				...
				// Step 4.3.1: If the command is status,
				...
				// Step 4.4.1: If the command is not one of the above,
				// it is invalid. Tell the user it is invalid.
				System.out.println("Unknown command.");
			}

			// Step 5: Print information about the specified book.
			printBook(userBook);
			userBook = null;
			
			// Step 6: Ask the user if they want to continue. If yes,
